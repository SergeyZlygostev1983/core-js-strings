...
function countVowels(str) {
  ...
  vowels.forEach((vowel) => {
    if (str.toLowerCase().includes(vowel)) {
      counter += str.length - str.toLowerCase().replaceAll(vowel, '').length;
    }
  });
...
function isPalindrome(str) {
  const strModify = str.toLowerCase().replaceAll(/[^a-z]+/g, '');
  return strModify === strModify.split('').reverse().join('');
}
